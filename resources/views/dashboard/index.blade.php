        ...
        <div class="bg-white p-8 rounded-lg shadow-md w-full"> 
            ...
            <form action="{{route('profile.update')}}" method="POST" enctype="multipart/form-data">
                ...
                <x-inputs.file id="avatar" name="avatar" label="Upload Avatar" />
                    <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white px-4 py border rounded focus:outline-none">Save</button>
            </form>
        </div>
            ...
            <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
                ...
                <div class="flex space-x-3">
                    ...
                    <form method="POST" action="{{route('jobs.destroy', $job->id)}}?from=dashboard" onsubmit="return confirm('Are you sure that you want to delete this job?')">
                        ...
                        @method('DELETE')
                        <button
                        type="submit" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded text-sm">Delete</button>
                    </form>
                    <!-- End Delete Form -->
                </div>
            </div>
